...
// List tasks (with filters) - scoped to current organization
router.get('/', (req, res) => {
...
router.post('/', (req, res) => {
  ...
  const r = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(title, description || null, assigned_to || null, req.user.id, priority || 'medium', due_date || null, category || null);

  res.status(201).json({ id: r.lastInsertRowid, message: 'Görev oluşturuldu' });
});
...
// Delete task - scoped to current organization
...
// Performance stats - scoped to current organization
router.get('/stats/performance', (req, res) => {
  const orgId = req.user.organization_id;
  const users = db.prepare("SELECT id, name, role, department FROM users WHERE status = 'active' AND organization_id = ?").all(orgId);
...
// Users list (for assignment dropdowns) - scoped to organization
router.get('/users/list', (req, res) => {
  const orgId = req.user.organization_id;
  const users = db.prepare("SELECT id, name, email, role, department FROM users WHERE status = 'active' AND organization_id = ? ORDER BY name").all(orgId);
  res.json(users);
});
